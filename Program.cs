    ...
    class Program
    {
        ...
        */
        ...
        private static string RemoveVowels(string s)
        {
            try
            {
                ...
                String final_string = "";                       // converted the String to a Charectyer Array and checked whether each of the charecter is a vowel.
                                                                // If yes then removed the vowel from the char array and added each of the non vowels to the string
                
                char[] charArr = s.ToCharArray();
                for (int x = 0; x < s.Length; x++)
                {

                    if (!(charArr[x] == 'a' || charArr[x] == 'e' || charArr[x] == 'i' || charArr[x] == 'o' || charArr[x] == 'u' ||
                        charArr[x] == 'A' || charArr[x] == 'E' || charArr[x] == 'I' || charArr[x] == 'O' || charArr[x] == 'U'))
                        final_string += charArr[x];
                }
                return final_string;
            }
            catch (Exception)
       ...
       */
        ...
        private static bool ArrayStringsAreEqual(string[] bulls_string1, string[] bulls_string2)
        {
            ...
            {
                // write your code here.
                string s1 = string.Join("", bulls_string1);        // I had no idea on how to do this without String Api Functions
                string s2 = string.Join("", bulls_string2);         // Just appended all the values in each array to a String 
                                                                    //then used to == operator to compare the Strings
                if (s1 == s2)
                {
                    return true;
                }
        ...
        */
        ...
        private static int SumOfUnique(int[] bull_bucks)
        {
            try
            {
                ...
                Dictionary<int, int> calcDictionary =  new Dictionary<int, int>();
                for (int i = 0; i < bull_bucks.Length; i++)
                {                                                       // used a dictonary to store the key value pairs 
                                                                        // added the first element of array as a Key  to Dictonary with value  1
                                                                        // If same element is present one more time in the array, then for the key in dictonary change the value (increment)
                                                                        // Loop all the elements
                                                                        // All the keys with value as 1 are taken and added to get the Unique Element Sum
                    if (calcDictionary.ContainsKey(bull_bucks[i]))
                        calcDictionary[bull_bucks[i]] += 1;
                    else
                        calcDictionary.Add(bull_bucks[i], 1);
                }

                foreach (var kvp in calcDictionary)
                {
                    if (kvp.Value == 1)
                        result += kvp.Key;
                }
        ...
        */
        ...
        private static int DiagonalSum(int[,] bulls_grid)
        {
            ...
            {
                // write your code here.
                int RowsColoumns = bulls_grid.GetLength(0); ;
                int DiagonolSum = 0;

                for (int i = 0; i < RowsColoumns; i++)
                {
                    for (int j = 0; j < RowsColoumns; j++)
                    {
                        if (i == j)                   //first calculated sum of all the primary diagonol elements
                            DiagonolSum += bulls_grid[i, j];
                        if ((i + j) == (RowsColoumns - 1) && (i != j))  // added all the secondary diagonal elements expect the one already included
                            DiagonolSum += bulls_grid[i, j];
                    }
                }
                 return DiagonolSum;
        ...
        */
        ...
        private static string RestoreString(string bulls_string, int[] indices)
        {
            try
            {
                ...
                for (int i = 0; i < indices.Length; i++)              // the charecter position is taken from indices 
                                                                     // A new String is taken. on that new String each of the letter is added according to the indices
                {
        ...
        */
        ...
        private static string ReversePrefix(string bulls_string6, char ch)
        {
            try
            {
                String afteroperationstring = "";
                int indextostartoperation = 0;
                for (int i = 0; i < bulls_string6.Length; i++)
                {
                    if (bulls_string6[i] == ch)
                    {
                        indextostartoperation = i;  // here I am finding the index(charecter position) where the operation should be applied
                        break;
                    }
                }
                for (int i = indextostartoperation; i >= 0; i--)
                { 
                    afteroperationstring += bulls_string6[i];               // Applying the string reverse operation
                }
                for (int i = indextostartoperation + 1; i < bulls_string6.Length; i++)
                {
                    afteroperationstring += bulls_string6[i];                  // Appending the leftover part to the output String after  operatiion
                }
                return afteroperationstring;
            }
            catch (Exception)
            {
